    ...
    (fn []
      [:div.modal (if @is-active {:class "is-active"})
       ...
       [:div.modal-card
        ...
        [:footer.modal-card-foot
         [:button#do-login.button.is-info
          {:on-click #(do
                        (attempt-login {:username @username
                                        :pass @pass})
                        (reset! is-active false))} "Login"]

         [:button#do-signup.button.is-success
          {:on-click #(do
    ...
    (fn []
      [:div.has-text-left
        ...
        [:div.level-right.has-text-right
          [:button#login.button.is-info {:on-click #(swap! is-active not)} "Login"]]
...
(defn logout-pop-up
  ...
  [:div.modal (if @is-active {:class "is-active"})
   ...
   [:div.modal-card
    ...
    [:footer.modal-card-foot
     [:button#do-logout.button.is-danger {:on-click #(do
                                                       (rf/dispatch-sync [:logout])
                                                       (reset! is-active false))} "Logout"]

     [:button#do-cancel.button {:on-click #(reset! is-active false)} "Cancel"]]]])
